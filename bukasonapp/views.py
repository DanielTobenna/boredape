...
def contact(request):
	if request.method == 'GET':
		...
		if form.is_valid():
			...
			try:
				send_mail(name, "Investor {} has sent a message saying: {}".format(email, message),email, ['support@Assetfussion.org'])
...
def signup(request):
	...
	if form.is_valid():
		...
		#email_message= EmailMessage(
			#'Welcome on board to Assetfussion!',
			#template,
			#settings.EMAIL_HOST_USER,
			#[email],
...
def deposit(request):
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			try:
				send_mail(client_username, "A client with username: {} wants to make a deposit of {}".format(client_username, price_amount),client_email, ['support@Trustversexpert.com'])
			except BadHeaderError:
				pass
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'deposit' and float(client_deposit)>10:
			...
			if client_deposit_balance < 0:
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),client_email, ['support@Assetfussion.org'])
